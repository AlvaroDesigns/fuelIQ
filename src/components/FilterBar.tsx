...
import { EVConnectorType, EVPowerCategory } from "@/lib/types/ev";
...
interface Props {
  ...
  onSelectEVPowerCategory: (cat: EVPowerCategory) => void;
  evBatteryCapacity: number;
  onChangeEVBatteryCapacity: (kwh: number) => void;
  evSelectedConnectors: EVConnectorType[];
  onToggleEVConnector: (conn: EVConnectorType) => void;
}
...
export default function FilterBar({
  ...
  evPowerCategory,
  onSelectEVPowerCategory,
  evBatteryCapacity,
  onChangeEVBatteryCapacity,
  evSelectedConnectors,
  onToggleEVConnector,
}: Props) {
  ...
  const evPowerCategories: {
    id: EVPowerCategory;
    label: string;
    desc: string;
  }[] = [
    { id: "ALL", label: "Todas las Potencias", desc: "Cualquier cargador" },
    { id: "ULTRARAPID", label: "⚡ Ultrarrápida", desc: "≥ 150 kW" },
    { id: "RAPID", label: "⚡ Rápida", desc: "50 - 149 kW" },
    { id: "SEMIRAPID", label: "Semi-rápida / AC", desc: "11 - 49 kW" },
  ];

  const evConnectorsList: { type: EVConnectorType; label: string }[] = [
    { type: "CCS2", label: "CCS2 Combo" },
    { type: "TESLA_SUPERCHARGER", label: "Tesla Supercharger" },
    { type: "TYPE_2", label: "Tipo 2 Mennekes" },
  ...
  return (
    <>
      ...
      <div className="block md:hidden space-y-3">
        ...
        <div className="revolut-card rounded-2xl p-3 flex items-center gap-2 shadow-sm">
          ...
          >
            ...
            <div className="flex-1 truncate">
              ...
              <span className="text-[10px] text-slate-500 dark:text-zinc-400">
                Radio {radius} km ·{" "}
                {activeMode === "ev"
                  ? "Red Puntos de Recarga"
                  : FUEL_TYPES[selectedFuel].shortLabel}
              </span>
            </div>
          </button>
          ...
          <button
            ...
            className="w-10 h-10 rounded-full bg-slate-100 dark:bg-white/10 border border-slate-200 dark:border-white/10 text-slate-700 dark:text-zinc-300 flex items-center justify-center shrink-0 shadow-sm"
          >
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1 px-1 scrollbar-none">
          {activeMode === "fuel"
            ...
            : evPowerCategories.map((p) => {
                const isSelected = evPowerCategory === p.id;
                return (
                  <button
                    key={p.id}
                    onClick={() => onSelectEVPowerCategory(p.id)}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-extrabold whitespace-nowrap transition-all border shrink-0 ${
                      isSelected
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-black text-base text-slate-950 dark:text-white">
                    Filtros de Búsqueda (
                    {activeMode === "ev" ? "⚡ Eléctricos" : "⛽ Combustibles"})
                  </h3>
                  <p className="text-[11px] text-slate-500 dark:text-zinc-400 font-medium">
                    Ajusta tu radio, conectores y preferencias
                  </p>
                </div>
              </div>
            ...
            <div className="p-5 space-y-5 overflow-y-auto flex-1">
                ...
                <div className="relative flex items-center bg-slate-100 dark:bg-black/60 border border-slate-200 dark:border-white/15 rounded-2xl p-1.5">
                  ...
                  {searchInput && (
                    <button
                      ...
                      className="p-1 mr-1 text-slate-400"
                    >
                      <X className="w-4 h-4" />
                    </button>
                  )}
                  ...
                  <div className="absolute top-full left-0 right-0 mt-1 z-50 bg-white dark:bg-[#12161f] border border-slate-200 dark:border-white/15 rounded-2xl shadow-xl overflow-hidden max-h-48 overflow-y-auto">
                    {searchResults.map((item, idx) => (
                      ...
                      >
                        ...
                        {item.postalCode && (
                          <span className="text-[10px] text-[#0075FF] font-black">
                            CP {item.postalCode}
                          </span>
                        )}
                      </button>
              ...
              {activeMode === "ev" ? (
                <>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-[11px] font-black uppercase tracking-wider text-slate-600 dark:text-zinc-400">
                      ...
                      <span className="text-cyan-500 font-black">
                        {evBatteryCapacity} kWh
                      </span>
                    </div>
                    <div className="grid grid-cols-5 gap-1.5">
                      {[40, 50, 60, 77, 100].map((kwh) => (
                        <button
                          key={kwh}
                          onClick={() => onChangeEVBatteryCapacity(kwh)}
                          className={`py-2 rounded-xl text-xs font-black border transition-all ${
                            evBatteryCapacity === kwh
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {evConnectorsList.map((conn) => {
                        const isSelected = evSelectedConnectors.includes(
                          conn.type,
                        );
                        return (
                          <button
                            key={conn.type}
                            onClick={() => onToggleEVConnector(conn.type)}
                            className={`p-2.5 rounded-2xl text-xs font-bold border flex items-center justify-between transition-all ${
                              isSelected
                                ? "bg-cyan-500/15 border-cyan-500 text-cyan-600 dark:text-cyan-300"
                                : "bg-slate-100 dark:bg-white/5 border-slate-200 dark:border-white/10 text-slate-700 dark:text-zinc-300"
                            }`}
                          >
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-[11px] font-black uppercase tracking-wider text-slate-600 dark:text-zinc-400">
                    ...
                    <span className="text-[#00D97E] font-black">
                      {tankCapacity} Litros
                    </span>
                  </div>
                  <input
                    ...
                    value={tankCapacity}
                    onChange={(e) =>
                      onChangeTankCapacity(parseInt(e.target.value, 10))
                    }
                    className="w-full h-2 bg-slate-200 dark:bg-zinc-800 rounded-lg appearance-none cursor-pointer accent-[#00D97E]"
                  />
                </div>
              )}
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  >
                    ⭐ Mejor precio por {activeMode === "ev" ? "kWh" : "litro"}
                  </option>
                  <option
                    value="tankSaving"
                    className="bg-white dark:bg-[#0f1117]"
                  ...
                  <option
                    ...
                    className="bg-white dark:bg-[#0f1117]"
                  >
                    🧠 Smart Score (Carga + Desplazamiento)
                  </option>
                </select>
              </div>
      ...
      <div className="hidden md:block revolut-card rounded-[2.5rem] p-6 sm:p-7 space-y-6">
          ...
          <div ref={searchContainerRef} className="relative flex-1">
            <div className="relative flex items-center bg-slate-100/90 dark:bg-black/70 border border-slate-200 dark:border-white/15 hover:border-slate-300 dark:hover:border-white/30 focus-within:border-[#0075FF] focus-within:ring-2 focus-within:ring-[#0075FF]/20 rounded-full transition-all p-1.5 shadow-sm">
              ...
              <input
                ...
                placeholder={
                  activeMode === "ev"
                    ? "Buscar puntos de recarga por CP (ej. 07001), municipio o calle..."
                    : "Buscar gasolineras por CP (ej. 07001), municipio o calle..."
                }
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none">
            ...
            {QUICK_CITIES.map((c) => {
              ...
              return (
                ...
                >
                  {c.name}{" "}
                  <span className="opacity-60 text-[10px]">({c.cp})</span>
                </button>
              );
            })}
          </div>
        ...
        {/* Dynamic Category Selector: Fuel Types or EV Power & Connectors */}
        {activeMode === "fuel" ? (
          ...
          <div className="space-y-2.5">
            <div className="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400 flex items-center justify-between">
              <span className="flex items-center gap-1.5 font-black text-slate-700 dark:text-zinc-300">
                <Fuel className="w-3.5 h-3.5 text-[#00D97E]" /> Tipo de
                Carburante
              </span>
            ...
            <div className="flex items-center justify-between gap-4 flex-wrap">
              <div className="space-y-1.5">
                <div className="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400 flex items-center gap-1.5">
                  <Zap className="w-3.5 h-3.5 text-cyan-500" /> Potencia de
                  Carga
                </div>
                <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none">
                  {evPowerCategories.map((p) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-full text-xs font-black whitespace-nowrap transition-all border ${
                          isSelected
                            ? "bg-cyan-500 text-black border-cyan-500 shadow-lg scale-[1.02]"
                            : "bg-slate-100/80 dark:bg-white/[0.03] border-slate-200 dark:border-white/10 text-slate-700 dark:text-zinc-300 hover:text-slate-900 dark:hover:text-white"
                        }`}
                      >
                        <Zap className="w-3 h-3" />
                        <span>{p.label}</span>
              ...
              <div className="space-y-1.5">
                <div className="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400 flex items-center gap-1.5">
                  <Plug className="w-3.5 h-3.5 text-cyan-500" /> Conectores
                </div>
                <div className="flex items-center gap-1.5">
                  {evConnectorsList.map((conn) => {
                    const isSelected = evSelectedConnectors.includes(conn.type);
                    return (
                      <button
                        key={conn.type}
                        onClick={() => onToggleEVConnector(conn.type)}
                        className={`px-3 py-1.5 rounded-full text-[11px] font-bold transition-all border flex items-center gap-1 ${
                          isSelected
                            ? "bg-cyan-500/15 border-cyan-500 text-cyan-700 dark:text-cyan-300 font-black"
                            : "bg-slate-100/80 dark:bg-white/[0.03] border-slate-200 dark:border-white/10 text-slate-600 dark:text-zinc-400"
                        }`}
                      >
                        <span>{conn.label}</span>
        ...
        {/* Bottom Row: Deposit/Battery Slider & Sorting Criteria */}
        <div className="pt-4 border-t border-slate-200/60 dark:border-white/5 flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-5">
          ...
          {activeMode === "fuel" ? (
            <div className="flex items-center gap-4 bg-slate-100/70 dark:bg-white/[0.02] border border-slate-200 dark:border-white/10 p-3.5 rounded-2xl flex-1 max-w-xl shadow-inner">
              ...
              <div className="flex-1 space-y-1.5">
                <div className="flex justify-between text-xs font-bold text-slate-700 dark:text-zinc-300">
                  ...
                  <span className="text-[#00A860] dark:text-[#00D97E] font-black text-sm">
                    {tankCapacity} Litros
                  </span>
                </div>
                <input
            ...
            <div className="flex items-center gap-4 bg-slate-100/70 dark:bg-white/[0.02] border border-slate-200 dark:border-white/10 p-3.5 rounded-2xl flex-1 max-w-xl shadow-inner">
              ...
              <div className="flex-1 space-y-1.5">
                <div className="flex justify-between text-xs font-bold text-slate-700 dark:text-zinc-300">
                  <span>Batería de tu coche eléctrico</span>
                  <span className="text-cyan-500 font-black text-sm">
                    {evBatteryCapacity} kWh (Carga 10% → 80%)
                  </span>
                </div>
                <input
                  type="range"
                  ...
                  onChange={(e) =>
                    onChangeEVBatteryCapacity(parseInt(e.target.value, 10))
                  }
                  className="w-full h-1.5 bg-slate-300 dark:bg-zinc-800 rounded-lg appearance-none cursor-pointer accent-cyan-500"
                />
              </div>
            </div>
          )}
          ...
          <div className="flex items-center gap-2.5 self-start lg:self-auto">
            ...
            >
              <option
                value="finalPrice"
                className="bg-white dark:bg-[#0f1117] text-slate-900 dark:text-white"
              >
                ⭐ Mejor precio por {activeMode === "ev" ? "kWh" : "litro"}
              </option>
              <option
